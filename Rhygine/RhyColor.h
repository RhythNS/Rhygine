...
namespace RhyC
{
	...
	/// <summary>
	/// Creates a color from float values.
	/// </summary>
	/// <param name="r">The red value.</param>
	/// <param name="g">The green value.</param>
	/// <param name="b">The blue value.</param>
	...
	{
		color col = static_cast<short>(r * 255.0f);
		col |= (static_cast<short>(g * 255.0f) << 8);
		col |= (static_cast<short>(b * 255.0f) << 16);
		col |= (static_cast<short>(a * 255.0f) << 24);
		return col;
	}
