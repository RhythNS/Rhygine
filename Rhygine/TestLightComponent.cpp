...
void TestLightComponent::Update()
{
	...
	ImGui::DragFloat3("Position", transform->position, 0.1f, -10.0f, 10.0f, "%.3f", 0);
	if (ImGui::DragFloat3("Rotation", direction.m_floats, 0.01f, -1.0f, 1.0f, "%.3f", 0)) {
		transform->rotation = RhyM::Quat(direction.m_floats[0] * RhyM::PI, direction.m_floats[1] * RhyM::PI, direction.m_floats[2] * RhyM::PI);
	}
	ImGui::End();
}
