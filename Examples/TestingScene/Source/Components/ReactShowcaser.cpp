...
void ReactShowcaser::Init()
{
	boxTexture = std::make_unique<Texture>("Assets\\PresentScene\\box.png", 0);

	RhyReact::RigidBody* ground = CreateBox(baseSpawnPoint - RhyM::Vec3(0.0f, 3.0f, 0.0f), RhyM::Vec3(5.0f, 1.0f, 5.0f), 0.0f);
	ground->SetType(reactphysics3d::BodyType::KINEMATIC);
}

void ReactShowcaser::Update()
{
	timer -= GetDelta();
	...
	RhyReact::RigidBody* box = CreateBox(baseSpawnPoint + position, RhyM::Vec3(1.0f, 1.0f, 1.0f), 80.0f);
	box->GetGameObject()->AddComponent<DeleteWhenBelow>();
}

RhyReact::RigidBody* ReactShowcaser::CreateBox(RhyM::Vec3 position, RhyM::Vec3 size, float mass)
{
	...
	RhyReact::RigidBody* body = go->AddComponent<RhyReact::RigidBody>();
	
	RhyReact::BoxCollider* boxCollider = new RhyReact::BoxCollider(size * 0.5f);
	body->AddCollider(boxCollider, RhyM::Vec3::Zero(), RhyM::Quat::Identity(), true);
	boxCollider->GetMaterial().setBounciness(0.1f);
